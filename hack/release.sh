...
ytt -f config/. -v controller_image=${controller_image} -v webhook_image=${webhook_image}  -v build_init_image=${build_init_image} -v rebase_image=${rebase_image} -v nop_image=${nop_image} > ${release_yaml}