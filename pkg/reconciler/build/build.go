...
func buildMetadataFromBuiltImage(image cnb.BuiltImage) []v1alpha2.BuildpackMetadata {
	buildpackMetadata := make([]v1alpha2.BuildpackMetadata, 0, len(image.BuildpackMetadata))
	for _, metadata := range image.BuildpackMetadata {
		buildpackMetadata = append(buildpackMetadata, v1alpha2.BuildpackMetadata{
			Id:      metadata.ID,
