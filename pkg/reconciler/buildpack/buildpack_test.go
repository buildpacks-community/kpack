...
import (
	...
	"github.com/pivotal/kpack/pkg/reconciler/buildpack/buildpackfakes"
	"github.com/pivotal/kpack/pkg/reconciler/testhelpers"
	"github.com/pivotal/kpack/pkg/registry"
	"github.com/pivotal/kpack/pkg/registry/registryfakes"
)

func TestBuildpackReconciler(t *testing.T) {
	spec.Run(t, "Buildpack Reconciler", testBuildpackReconciler)
}

func testBuildpackReconciler(t *testing.T, when spec.G, it spec.S) {
	const (
		testNamespace           = "some-namespace"
		buildpackName           = "some-buildpack"
		buildpackKey            = testNamespace + "/" + buildpackName
		initialGeneration int64 = 1
	)
	var (
		fakeStoreReader     = &buildpackfakes.FakeStoreReader{}
	...
	when("#Reconcile", func() {
		...
		it("saves metadata to the status", func() {
			...
			assert.Equal(t, []corev1alpha1.StoreImage{bp.Spec.Source}, StoreSpec)
		})
		...
		it("uses the keychain of the referenced service account", func() {
			...
			bp.Spec.ServiceAccountName = serviceAccountName
		...
		it("does not update the status with no status change", func() {
			...
			bp.Status = buildapi.BuildpackStatus{
				Status: corev1alpha1.Status{
					ObservedGeneration: 1,
					Conditions: corev1alpha1.Conditions{
						{
