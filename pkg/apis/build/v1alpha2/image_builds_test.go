...
func testImageBuilds(t *testing.T, when spec.G, it spec.S) {
	...
	when("#build", func() {
		...
		it("generates a build name with build number", func() {
			...
			build, err := image.Build(sourceResolver, builder, latestBuild, "", "", 27, "")
		...
		it("sets build priority class correctly", func() {
			build, err := image.Build(sourceResolver, builder, latestBuild, "some-reasons", "some-changes", 27, "some-class")
			require.NoError(t, err)
		...
		it("propagates image's annotations onto the build", func() {
			build, err := image.Build(sourceResolver, builder, latestBuild, "some-reasons", "some-changes", 27, "")
			require.NoError(t, err)
			assert.Equal(t, map[string]string{"annotation-key": "annotation-value", "image.kpack.io/buildChanges": "some-changes", "image.kpack.io/reason": "some-reasons","image.kpack.io/latestBuildpackMetadata":`[{"id":"buildpack.matches","version":"1"}]`}, build.Annotations)
		})
		...
		it("with excludes additional autogenerated tags names when explicitly disabled", func() {
			...
			build, err := image.Build(sourceResolver, builder, latestBuild, "", "", 1, "")
			require.NoError(t, err)
		...
		when("generates additional image names for a provided build number", func() {
			it("with tag prefix if image name has a tag", func() {
				...
				build, err := image.Build(sourceResolver, builder, latestBuild, "", "", 45, "")
				require.NoError(t, err)
		...
		it("adds build reasons and changes annotation", func() {
			...
			build, err := image.Build(sourceResolver, builder, latestBuild, reasons, changes, 1, "")
			require.NoError(t, err)
