...
func testBuildPod(t *testing.T, when spec.G, it spec.S) {
	...
	when("BuildPod", func() {
		when(">= 0.2 platform api", func() {
			...
			it("uses v1alpha1 bindings when no services are configured and the v1alpha1 bindings annotation is set", func() {
				build.Spec.Services = nil
				build.Annotations[v1alpha2.V1Alpha1BindingsAnnotation] = `[{"name":"apm","metadataRef":{"name":"apm-configmap"},"secretRef":{"name":"apm-secret"}},{"name":"database","metadataRef":{"name":"database-configmap"}}]`
				pod, err := build.BuildPod(config, secrets, buildPodBuilderConfig)
				require.NoError(t, err)
