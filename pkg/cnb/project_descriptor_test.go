...
uri = "check-this-out.com"
		...
		when("the descriptor has groups", func() {
			it.Before(func() {
				ioutil.WriteFile(projectToml, []byte(`
[[build.groups]]
id = "cool-buildpack"
...
uri = "check-this-out.com"
		...
		when("the descriptor has buildpacks using the buildpacks key", func() {
			it.Before(func() {
				ioutil.WriteFile(projectToml, []byte(`
[_]
schema-version = "0.2"

[[io.buildpacks.buildpacks]]
id = "cool-buildpack"
version = "v4.2"
uri = "check-this-out.com"
				`), 0644)
			})
			it("ignores the buildpacks key", func() {
				assert.Nil(t, cnb.ProcessProjectDescriptor(appDir, platformDir, logger))
				assert.Equal(t, "", buf.String())
			})
		})
