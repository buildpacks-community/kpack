...
type ClusterBuilderLister interface {
	...
	List(selector labels.Selector) (ret []*buildv1alpha1.ClusterBuilder, err error)
	// Get retrieves the ClusterBuilder from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*buildv1alpha1.ClusterBuilder, error)
...
type clusterBuilderLister struct {
	listers.ResourceIndexer[*buildv1alpha1.ClusterBuilder]
}
...
func NewClusterBuilderLister(indexer cache.Indexer) ClusterBuilderLister {
	return &clusterBuilderLister{listers.New[*buildv1alpha1.ClusterBuilder](indexer, buildv1alpha1.Resource("clusterbuilder"))}
}
